...
class multifidelityUCB:
    ...
    def run_bayes_loop(self, n_iter, min_hf_evals=1, plot_opt=False, plot_acq=False, is_GP=True):
        ...
        for i in range(n_iter):
            ...
            # _, _, error2 = self.acquisition.acquisition_low_high(self.x_plot_low, self.x_plot_high)
            # error = np.max(error2)
            ...
            if self.beta * np.sqrt(variance_l) > self.gamma:
                ...
                if self.normalize:
                    ynew = self.scaler_low.transform(ynew)
                ...
                ynew = self.multifidelity.f[fidelity](xnext)
                if self.normalize:
                    ynew = self.scaler_high.transform(ynew)
            ...
            if self.normalize:
                ...
                # y = self.scaler.inverse_transform(self.model.Y)
    ...
    def plot_acquisition(self, xnew):
        ...
        if self.ndim==1:
            ...
            gamma = error*np.sqrt(self.cost_ratio)
            plt.plot(self.x_plot_low[:, 0], mean_l + gamma + error, color='k', linestyle='--', label='Gamma threshold')
...
class CustomMultiFidelityUCB(Acquisition):
    def __init__(self, model: IModel, space, ei_beta=1):
        """
        Custom Multi-fidelity UCB
        
        :model: Multi-fidelity model
        :space: Search space over which the optimization is performed
        :exploration parameter: Optional exploration parameter
        """
        super().__init__()
    ...
    def acquisition_low_high(self, x_low, x_high):
        ...
        return [mean_l + self.ei_beta* np.sqrt(variance_l) + error,
               mean_h + self.ei_beta * np.sqrt(variance_h),
               error]
