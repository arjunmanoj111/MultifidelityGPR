...
class multifidelityUCB:
    ...
    def set_model(self, x_array=None, y_array=None, ker='rbf'):
        ...
        if self.ndim==1:
            lb, ub = self.bounds
            self.parameter_space = ParameterSpace([ContinuousParameter('x', lb, ub)])
        else:
            paramspace = []
            for i in range(self.ndim):
                lb, ub = self.bounds[i]
        ...
        if ker=='rbf':
            ...
            kernel.lengthscale.constrain_bounded(0.01, 0.5, warning=False)
            
        else:
            kernel = GPy.kern.Matern52(self.ndim)
            kernel.lengthscale.constrain_bounded(0.005, 0.5, warning=False)
            
        gpy_model = GPy.models.GPRegression(x_array, y_array, kernel, noise_var=self.noise)
